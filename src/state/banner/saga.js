...
function* watchUpdateBanner({ payload }) {
  const res = yield call(client.put, `${Endpoint.BANNER}/${payload.bannerId}`, {
    is_active: payload.checked,
    order: payload.order,
  });
  yield put(updateBannerFinish(res));
  return yield fork(() =>
    watchGetBanner({ payload: payload.filterConditions })
  );
}
function* watchDeleteBanner({ payload }) {
  const res = yield call(
    client.delete,
    `${Endpoint.BANNER}/${payload.bannerId}`
  );
  yield put(updateBannerFinish(res));
  return yield fork(() =>
    watchGetBanner({ payload: payload.filterConditions })
