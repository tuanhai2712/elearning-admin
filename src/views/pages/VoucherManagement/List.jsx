...
import { Button, Row, Spin, Col, Pagination, Modal, Radio } from 'antd';
...
export default function List() {
  ...
  const [visibleModalCreateCoupon, setVisibleModalCreateCoupon] = useState(
    false
  );
  ...
  const checkActiveCoupon = (activeTo) => {
    if (
      new Date().setHours(0, 0, 0, 0) <= new Date(activeTo).setHours(0, 0, 0, 0)
    )
      return true;
    return false;
  };
      ...
      return data.map((item) => {
        return (
          <Col span={6} key={item.id} style={{ padding: 5 }}>
            <Spin
              spinning={item.id === couponUpdate.id && couponUpdate.loading}
            >
              <img
                src={`${BASE_LOCAL_URL}/${item.thumbnail}`}
                alt={item.id}
                style={{
                  width: '100%',
                  height: 200,
              ...
              >
                <div>
                  <Button
                    ...
                    style={{ padding: 'unset', marginRight: 10 }}
                    icon={<DeleteOutlined />}
                  ></Button>
                  {checkActiveCoupon(item.active_to) && (
                    <Button
                      onClick={() => openEditCoupon(item)}
                      style={{ padding: 'unset' }}
                      icon={<ToolOutlined />}
                    ></Button>
                  )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <PointStyled>Điểm: {item.point}</PointStyled>
                <TimeActiveTextStyled>
                  Ngày hết hạn: {moment(item.active_to).format('DD-MM-YYYY')}
                </TimeActiveTextStyled>
              </div>
            </Spin>
          </Col>
  ...
  return (
    <Fragment>
        ...
        <Row style={{ marginBottom: 10 }} justify="space-between">
          <Col span={6}>
            <Radio.Group
              ...
              value={filterConditions.isActive ? 1 : 0}
            >
      ...
      {visibleModalCreateCoupon && (
        <ModalCreateCoupon
          visible={visibleModalCreateCoupon}
          action={() => openCreateBannerModal()}
          filterConditions={filterConditions}
        />
      )}
      {visibleModalEditCoupon && (
        <ModalEditCoupon
          visible={visibleModalEditCoupon}
          action={() => closeEditCoupon()}
          data={couponEdit}
          filterConditions={filterConditions}
        />
      )}
