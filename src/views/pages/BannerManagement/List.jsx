...
export default function List() {
  ...
  const [visibleModalCreateBanner, setVisibleModalCreateBanner] = useState(
    false
  );
      ...
      return data.map((item) => {
        return (
          <Col span={6} key={item.id} style={{ padding: 5 }}>
            <Spin
              spinning={item.id === bannerUpdate.id && bannerUpdate.loading}
            >
              <img
                src={`${BASE_LOCAL_URL}/${item.image}`}
                alt={item.id}
                style={{
                  width: '100%',
                  height: 200,
              ...
              >
                ...
                <Switch
                  ...
                  onChange={(checked) => handleActiveBanner(checked, item)}
                  checkedChildren="Bỏ kích hoạt"
  ...
  return (
    <Fragment>
      ...
      {visibleModalCreateBanner && (
        <ModalCreateBanner
          visible={visibleModalCreateBanner}
